...
실행 중인 ComfyUI 작업은 현재 노드와 단계 진행률을 대기열·생성 이력에 실시간으로 반영합니다. 후처리 완료 상태도 이벤트로 갱신되므로 결과가 준비되면 별도 새로고침 없이 확인할 수 있습니다.
...
로그인한 사용자는 자신의 생성 권한 범위에서 활성 큐와 진행률을 볼 수 있습니다. 공개 워크플로우는 역할·사용자별 활성 작업 상한을 적용해 한 사용자가 전체 큐를 점유하지 않도록 제한할 수 있습니다.
