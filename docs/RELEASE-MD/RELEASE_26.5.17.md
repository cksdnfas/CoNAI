...
v26.5.17은 `26.4.27` 이후 누적된 218개 커밋을 압축한 알파 릴리즈입니다. 핵심은 워크플로우 그래프 실행 안정화, 이미지/미디어 상세 경험 개선, ComfyUI 메타데이터 추출 보강, 데이터 재매칭 유지보수, 한국어 기본 UI, 앱 메타데이터/브랜딩, 그리고 대규모 리팩터링/계약 검증 보강입니다.
...
- 이미지 상세 필터 preview의 초기 선택/전환 상태를 안정화해 잘못된 preview 표시를 방지
- 이미지 다운로드 API와 브라우저 fallback 흐름을 보강해 저장 실패/파일명 누락을 줄임
...
- 썸네일·메타데이터·해시 재생성용 데이터 재매칭 유지보수 워크플로우 추가
- 해시 재생성 중 maintenance lock을 적용하고 stale hash-linked relation 정리 흐름 보강
...
ComfyUI 워크플로우에서 저장 이미지로 이어지는 실제 입력 경로를 더 정확히 추적하도록 보강했습니다.

- 저장 이미지 branch를 custom text/source node까지 따라가 prompt/source 값을 추출
- stale cached result보다 live `StringFunction` 입력을 우선해 이전 값이 잘못 추출되는 문제 완화
- `rgthree` sampler config, seed, switch, random-size output resolve 지원
- ComfyUI metadata extraction 검증 스크립트 추가
...
- 앱 title/description, favicon, touch icon, web manifest 등 문서 메타데이터 추가
- 좌측 brand icon을 고해상도 앱 아이콘으로 직접 표시해 이중 패딩/축소 표시 방지
...
- ComfyUI metadata extraction, image download, image detail filter preview, data rematch, app document metadata 계약 검증 추가

---

### 포함된 주요 커밋 범위

이 문서는 Git 태그 `26.4.27` 이후 `8043aa9a`까지의 변경을 바탕으로 정리했습니다.

- 커밋 범위: `26.4.27..8043aa9a`
- 커밋 수: **218**
- 변경 규모: **637 files changed, 76,990 insertions(+), 16,895 deletions(-)**
- 대표 커밋:
  ...
  - `ea00d904` Stabilize image detail filter preview
  - `bdbbfc43` Restore reliable image downloads
  - `259b3148` Handle ComfyUI custom node metadata extraction
  - `58873135` Add data rematch maintenance workflow
  - `8043aa9a` Add app metadata and favicon assets
...
- `npm run verify:data-rematch-contracts`
- `cd frontend && npx tsx src/scripts/verifyAppDocumentMetadataContracts.ts`
