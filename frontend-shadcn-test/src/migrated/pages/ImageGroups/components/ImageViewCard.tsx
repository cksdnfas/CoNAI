...
export function ImageViewCard({ group, onClick }: ImageViewCardProps) {
  ...
  // 이미지 가져오기 함수 (하위 그룹 제외)
  const fetchImages = useCallback(
    async (count: number): Promise<ImageRecord[]> => {
      try {
        const response = await groupApi.getPreviewImages(group.id, count, false); // includeChildren = false
  ...
  const { currentImage, images, isLoading } = useImageRotation(fetchImages, {
    groupId: `${group.id}-direct`, // 캐시 키 (하위 그룹 포함 버전과 구분)
    ...
    >
      ...
      >
        <Typography variant="h6" sx={{ color: 'white', fontWeight: 600 }}>
          {t('imageView.title', { name: group.name })}
        </Typography>
