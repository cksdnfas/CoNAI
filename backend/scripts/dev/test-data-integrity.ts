...
const both = db.prepare(`
  SELECT COUNT(*) as count
  FROM media_metadata mm
  INNER JOIN image_files if_ ON mm.composite_hash = if_.composite_hash
`).get() as { count: number };
