...
ok(appShell.includes('to="/access"'), 'header brand link should open the access overview')
ok(appShell.includes("prefetchAppRoute('/access')"), 'header brand link should prefetch the access overview')
ok(appShell.includes("const PRIMARY_NAV_ORDER = ['/', '/groups'"), 'primary nav should include the image gallery route')
ok(appShell.includes("icon: MapIcon"), 'access overview nav should use a page-map icon')
ok(!appShell.includes('ShieldCheck'), 'access overview nav should not use a security-oriented shield icon')
ok(appShell.includes('item.permissionKey === null || hasAuthPermission'), 'access overview nav should stay visible before sign-in')
