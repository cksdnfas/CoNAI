import { ok } from 'node:assert/strict'
import { readFileSync } from 'node:fs'
import { join } from 'node:path'

const root = process.cwd()
const settingsTabs = readFileSync(join(root, 'src/features/settings/settings-tabs.ts'), 'utf8')
const settingsTabNav = readFileSync(join(root, 'src/features/settings/components/settings-tab-nav.tsx'), 'utf8')
const settingsPage = readFileSync(join(root, 'src/features/settings/settings-page.tsx'), 'utf8')
const releaseReadinessTab = readFileSync(join(root, 'src/features/settings/components/release-readiness-tab.tsx'), 'utf8')

ok(settingsTabs.includes("'release-readiness'"), 'settings tab union should include release-readiness')
ok(settingsTabs.includes("{ value: 'release-readiness' }"), 'settings tab list should expose release-readiness')
ok(settingsTabNav.includes("'release-readiness': { ko: '릴리즈 준비', en: 'Release readiness' }"), 'settings nav should label release-readiness')
ok(settingsPage.includes("import('./components/release-readiness-tab')"), 'settings page should lazy-load the release readiness tab')
ok(settingsPage.includes("activeTab === 'release-readiness'"), 'settings page should render the release readiness tab')
ok(releaseReadinessTab.includes('REVIEW_ITEMS'), 'release readiness workspace should have local review checklist items')
ok(releaseReadinessTab.includes("'completed-work'"), 'release readiness checklist should include completed work review')
ok(releaseReadinessTab.includes("'caveats'"), 'release readiness checklist should include caveat review')
ok(releaseReadinessTab.includes("'evidence'"), 'release readiness checklist should include evidence review')
ok(releaseReadinessTab.includes("'decisions'"), 'release readiness checklist should include user-owned decision review')
ok(releaseReadinessTab.includes('EVIDENCE_ITEMS'), 'release readiness workspace should expose evidence tiles')
ok(releaseReadinessTab.includes('npm run verify:release-readiness'), 'release readiness evidence should include the canonical release check')
ok(releaseReadinessTab.includes('python -m graphify update .'), 'release readiness evidence should include Graphify refresh')
ok(releaseReadinessTab.includes('USER_DECISIONS'), 'release readiness workspace should separate user-owned release decisions')
ok(releaseReadinessTab.includes("'alpha-push'"), 'release decisions should include alpha push approval')
ok(releaseReadinessTab.includes("'demo-update'"), 'release decisions should include demo host update approval')
ok(releaseReadinessTab.includes("'restart-smoke'"), 'release decisions should include restart/smoke approval')
ok(releaseReadinessTab.includes("'cleanup'"), 'release decisions should include destructive cleanup approval')
ok(releaseReadinessTab.includes('HANDOFF_EVIDENCE_ITEMS'), 'release readiness workspace should expose handoff evidence capture items')
ok(releaseReadinessTab.includes("'local-commit-snapshot'"), 'handoff evidence should include a local commit snapshot')
ok(releaseReadinessTab.includes('git status --short --branch'), 'handoff evidence should include the git status snapshot command')
ok(releaseReadinessTab.includes('git log --oneline origin/alphatest..HEAD'), 'handoff evidence should include the local commit range command')
ok(releaseReadinessTab.includes("'demo-host-handoff'"), 'handoff evidence should include demo host handoff notes')
ok(releaseReadinessTab.includes("'smoke-evidence'"), 'handoff evidence should include smoke evidence preparation')
ok(releaseReadinessTab.includes("'rollback-plan'"), 'handoff evidence should include rollback preparation')
ok(releaseReadinessTab.includes('RUNBOOK_GUARDRAILS'), 'release readiness workspace should expose runbook guardrails')
ok(releaseReadinessTab.includes("'protected-3999'"), 'runbook guardrails should include protected port 3999')
ok(releaseReadinessTab.includes("'rollback-before-restart'"), 'runbook guardrails should require rollback notes before restart')
ok(releaseReadinessTab.includes('Preparation only'), 'runbook guardrails should mark demo host actions as preparation only')
ok(releaseReadinessTab.includes('OPERATION_STEPS'), 'release readiness workspace should expose approval-gated operation steps')
ok(releaseReadinessTab.includes('data-release-readiness-operation-checklist="true"'), 'release readiness workspace should render the operation checklist')
ok(releaseReadinessTab.includes('git push origin alphatest'), 'operation checklist should include the alpha push command')
ok(releaseReadinessTab.includes('git pull --ff-only origin alphatest'), 'operation checklist should include the demo host fast-forward update command')
ok(releaseReadinessTab.includes('live target 2999 only'), 'operation checklist should keep smoke scoped to live target 2999')
ok(releaseReadinessTab.includes('data-release-readiness-runbook-export="true"'), 'release readiness workspace should expose exportable runbook history')
ok(releaseReadinessTab.includes('data-release-readiness-automation-context="true"'), 'release readiness workspace should expose automation context handoff')
ok(releaseReadinessTab.includes('data-release-readiness-operator-evidence-console="true"'), 'release readiness workspace should expose the operator evidence review console')
ok(releaseReadinessTab.includes("'mcp-dry-run-evidence-packet'"), 'operator evidence console should include MCP dry-run evidence')
ok(releaseReadinessTab.includes("'workflow-recovery-handoff-packet'"), 'operator evidence console should include workflow recovery handoff evidence')
ok(releaseReadinessTab.includes("'media-approval-packet'"), 'operator evidence console should include media approval packet evidence')
ok(releaseReadinessTab.includes('docs/systems/local-automation-context-operations-map.md'), 'automation context handoff should link the local operations map')
ok(releaseReadinessTab.includes('docs/GUIDE/MCP_GUIDE.md + backend/src/mcp/*'), 'automation context handoff should keep MCP entry points opt-in and reviewable')
ok(releaseReadinessTab.includes('data-release-readiness-history-record={record.id}'), 'saved readiness runs should be individually reviewable')
ok(releaseReadinessTab.includes('data-release-readiness-handoff-output="true"'), 'release readiness workspace should render handoff output')
ok(!releaseReadinessTab.includes('buildApiUrl('), 'release readiness workspace should not call backend action endpoints')
ok(!releaseReadinessTab.includes('fetch('), 'release readiness workspace should not perform release side effects')
ok(releaseReadinessTab.includes('triggerBlobDownload'), 'release readiness workspace should provide local Markdown handoff export')

console.log('Release readiness workspace contracts verified.')
