...
equal(allSummary.totalWidgetCount, 7)
equal(allSummary.visibleWidgetCount, 7)
equal(allSummary.badgeText, '7')
deepEqual(allSummary.visibleFolders.map((folder) => `${folder.id}:${folder.widgets.length}`), ['realtime:2', 'images:4', 'notes:1'])

const imageSummary = getWallpaperWidgetLibrarySearchSummary(widgets, '이미지', ko)
equal(imageSummary.visibleWidgetCount, 4)
equal(imageSummary.badgeText, '4/7')
...
equal(typedSummary.badgeText, '1/7')
...
equal(emptySummary.badgeText, '0/7')
deepEqual(emptySummary.visibleFolders, [])

const visibleFolders = getWallpaperWidgetLibraryVisibleFolders(widgets, '실행', ko)
deepEqual(visibleFolders.flatMap((folder) => folder.widgets.map((widget) => widget.type)), ['queue-status'])
