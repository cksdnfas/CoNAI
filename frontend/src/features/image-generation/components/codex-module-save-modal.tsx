...
interface CodexModuleSaveModalProps {
  open: boolean
  moduleName: string
...
export function CodexModuleSaveModal({
  open,
  moduleName,
  moduleDescription,
  fieldOptions,
      ...
      <SettingsModalBody className="space-y-5">
        <div className="grid gap-4 md:grid-cols-2">
          <SettingsField label="모듈 이름">
            <Input value={moduleName} onChange={(event) => onModuleNameChange(event.target.value)} placeholder="Codex Image Module" />
          </SettingsField>
