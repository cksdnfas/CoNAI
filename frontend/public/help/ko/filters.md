...
대신 **오토태그 조건**을 사용하세요 (WD Tagger 필요).

### Q10. 오토태그가 없는 이미지는?

**A**:
- 오토태그 조건은 WD Tagger로 분석된 이미지에만 적용됩니다
- "오토태그 존재 = 없음" 조건으로 미분석 이미지를 찾을 수 있습니다
- 설정에서 WD Tagger를 활성화하고 이미지를 분석하세요

## 🔗 관련 도움말

- **[설정 가이드](setup.md)** - WD Tagger 설정 방법
- **검색 페이지** - 복합 필터를 사용한 이미지 검색
- **자동 수집** - 그룹에 자동으로 이미지 수집 설정
